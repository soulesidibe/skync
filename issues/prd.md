...
- `add <name> --repo <url> --src <path> --dest <path> [--remote <name>] [--ref <ref>]`: add to manifest and initialize. If `dest` is absent or empty, vendor fresh (base and local both equal upstream). If `dest` already contains files, adopt it via a two-way comparison against upstream: identical files no-op, text divergences get git-style markers in place, binary/type divergences leave dest's bytes intact and surface as non-marker pending conflicts, dest-only files stay, upstream-only files materialize. Base is seeded from upstream HEAD on both paths. A snapshot is taken before any adopt mutation; conflicts set `pendingSha = sha` and exit 2 (resolve via `skync resolve`, rollback via `skync rollback`).
